...
项目维护两个独立的 MCP 服务器入口点：

**1. `build/index.js` (Legacy Server)**
- 源文件：`src/index.ts`
- 特点：包含所有工具的内联实现 + 模块化工具适配层
- 代码量：~650行
- 用途：向后兼容，支持渐进式迁移
- 工具处理：部分工具直接实现（connect_devtools, get_current_page, get_page_snapshot, click, screenshot），其他工具通过适配器调用模块化实现

**2. `build/server.js` (Modern Server - 推荐)**
- 源文件：`src/server.ts`
- 特点：完全依赖模块化工具系统，代码简洁
- 代码量：~245行
- 用途：所有新项目和新配置
...
- 新用户和新项目：使用 `build/server.js`
- 需要向后兼容：使用 `build/index.js`
...
### 构建过程
1. TypeScript编译（`tsc`）
2. 自动设置可执行权限（`build/index.js` 和 `build/server.js`）
...
**代码版本**：v0.3.3（src/index.ts 和 src/server.ts）
